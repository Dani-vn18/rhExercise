...
def crear_y_poblar_db():
    ...
    (2, '10000002', 'María Gómez', '2023-02-15', '2024-02-15', 'Analista de Sistemas', ROUND(RANDOM() * (5000000 - 1400000) + 1400000, -3), 'Proyecto B', 2),
    (3, '10000003', 'Juan Rodríguez', '2023-03-20', '2024-09-20', 'Asistente Administrativo', ROUND(RANDOM() * (5000000 - 1400000) + 1400000, -3), 'Proyecto C', 3),
    (4, '10000004', 'Laura Ramírez', '2023-04-25', '2023-10-25', 'Conductor de Reparto', ROUND(RANDOM() * (5000000 - 1400000) + 1400000, -3), 'Proyecto D', 1),
    (5, '10000005', 'Andrés Herrera', '2023-05-30', '2024-05-30', 'Operario de Producción', ROUND(RANDOM() * (5000000 - 1400000) + 1400000, -3), 'Proyecto A', 2),
    (6, '10000006', 'Sofía Castro', '2023-06-15', '2024-12-15', 'Coordinador de Logística', ROUND(RANDOM() * (5000000 - 1400000) + 1400000, -3), 'Proyecto B', 3),
    (7, '10000007', 'Felipe Ruiz', '2023-07-20', '2024-01-20', 'Ingeniero de Software', ROUND(RANDOM() * (5000000 - 1400000) + 1400000, -3), 'Proyecto C', 1),
    (8, '10000008', 'Diana López', '2023-08-25', '2024-08-25', 'Gerente Administrativo', ROUND(RANDOM() * (5000000 - 1400000) + 1400000, -3), 'Proyecto D', 2),
    (9, '10000009', 'Ricardo Medina', '2023-09-30', '2025-03-30', 'Conductor de Carga', ROUND(RANDOM() * (5000000 - 1400000) + 1400000, -3), 'Proyecto A', 3),
    (10, '10000010', 'Valeria Sánchez', '2023-10-05', '2024-04-05', 'Operario de Ensamblaje', ROUND(RANDOM() * (5000000 - 1400000) + 1400000, -3), 'Proyecto B', 1)
    ON CONFLICT DO NOTHING;
    """)
...
def pagina_generacion_word():
    ...
    if st.button("Generar Word"):
        ...
        if cedula_filter.strip():
            query += f" AND cedula = '{cedula_filter.strip()}'"
        if nombre_filter.strip():
            query += f" AND nombre_completo ILIKE '%{nombre_filter.strip()}%'"
        ...
        try:
            ...
            if df.empty:
                ...
                st.warning("Se encontraron múltiples registros. Ajuste los filtros para obtener un único registro.")
            else:
                # Convertir el único registro en un diccionario
                registro = df.copy().iloc[0].to_dict()
                
                # Crear el diccionario de contexto para la plantilla
                context = {
                    "id": registro.get("id", ""),
                    "cedula": registro.get("cedula", ""),
                    "nombre_completo": registro.get("nombre_completo", ""),
                    "fecha_ingreso": registro.get("fecha_ingreso", ""),
                    "fecha_retiro": registro.get("fecha_retiro", ""),
                    "cargo": registro.get("cargo", ""),
                    "salario": f"{registro.get('salario', 0):,}",
                ...
                #st.write("Contexto para la plantilla:", context)
                ...
                buffer = io.BytesIO()
                doc.save(buffer)
                buffer.seek(0)  # Reinicia la posición del buffer
                
                st.success("Documento generado exitosamente.")
                
                # Botón de descarga del documento generado
                st.download_button(
